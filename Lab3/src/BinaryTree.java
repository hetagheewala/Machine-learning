...
public class BinaryTree {
	...
	public void add(BTNode node, BTNode iNode)
	{
		if(node.term.compareTo(iNode.term) > 0) {
			if(node.left != null) 
				...
				//System.out.println("Inserted " +  iNode.term + " to the left of node " + node.term);
			}
		}
			...
			if(node.right != null)
				...
				//System.out.println("Inserted " +  iNode.term + " to the right of node " + node.term);
			}
				
		}else if(node.term.compareTo(iNode.term) == 0) {
			
			if(iNode.docLists != null) {
				if(node.docLists != null) {
					if(!node.docLists.contains(new Integer(iNode.docLists.get(0))))
						node.docLists.addAll(iNode.docLists);
				}
				else {
					node.docLists =  new ArrayList<>();
					node.docLists.addAll(iNode.docLists);
				}
			}
		}
	...
	{		
		...
		if(n.term.length() >= key.length()) {
			if(n.term.substring(0, key.length()).compareTo(key) == 0) {
				matches.add(n);
